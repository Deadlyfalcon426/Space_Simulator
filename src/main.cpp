...
signed main() {
    ...
    double base_step ; std::cout<<"Time per step? -> "; std::cin>>base_step;
    std::string choice ; std::cout<<"Euler or RK4 numerical analysis? -> "; std::cin>>choice;
    int steps = std::round(time/base_step);
    int secs_per_frame = time / 600;//60 fps, 10s? 600 frames
    int counter = 0;
    double accumulator = 0.0;
    while(!WindowShouldClose() && counter<steps){
        accumulator += secs_per_frame;
        while(accumulator >= base_step && counter<steps){
            step(CelestialBodies, base_step, choice);
            accumulator-=base_step;
            counter++;
        }
        renderer.update_frame(CelestialBodies);
