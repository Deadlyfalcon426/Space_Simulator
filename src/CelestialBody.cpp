...
name(name), 
mass(mass), 
position(PhysicsVector3(x,y,z,"m")), 
...
name(name), 
mass(mass), 
position(PhysicsVector3(0,0,0,"m")), 
acceleration(0,0,0, "m/s^2"), 
velocity(0,0,0, "m/s"),
units_mass("kg")
{
...
PhysicsVector3 CelestialBody::get_velocity() const{
    return velocity;
}
PhysicsVector3 CelestialBody::get_acceleration() const{
    return acceleration;
}
PhysicsVector3 CelestialBody::get_position() const{
    return position;
}
...
void CelestialBody::set_velocity(PhysicsVector3 new_velocity){
...
void CelestialBody::set_acceleration(PhysicsVector3 new_acceleration){
...
void CelestialBody::update_position(PhysicsVector3 speed, double time){
    PhysicsVector3 new_distance = PhysicsVector3(speed.getX()*time, speed.getY()*time, speed.getZ()*time, "m");
    position = position.sum(new_distance);
}

void CelestialBody::update_position(PhysicsVector3 new_position){
    position.adjust(new_position);
}

PhysicsVector3 CelestialBody::get_gravitational_force(const CelestialBody& other) const{
    return get_gravitational_force(other, position, other.get_position());
}


PhysicsVector3 CelestialBody::get_gravitational_force(const CelestialBody& other, const PhysicsVector3& new_position, const PhysicsVector3& new_position_other) const{
    ...
    PhysicsVector3 diff = new_position_other.difference(new_position);
    ...
    PhysicsVector3 new_g_force = PhysicsVector3(new_x,new_y,new_z,"N");
    return new_g_force;
}
